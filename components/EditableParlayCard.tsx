...
export const EditableParlayCard: React.FC<EditableParlayCardProps> = ({ legs, onUpdateLegCondition, title }) => {
  ...
  return (
    <Card>
      <div className="p-6">
        <h3 className="text-lg font-semibold mb-4 text-indigo-400">{title}</h3>
        <div className="space-y-4">
          {legs.map((leg) => (
            <div key={leg.id} className="p-4 bg-gray-700 rounded-md shadow grid grid-cols-1 md:grid-cols-4 gap-3 items-center">
              ...
              <div className="md:col-span-2">
                {leg.isEditableOverUnder ? (
                  <Select
                    ...
                    className="w-full md:w-auto bg-gray-600 border-gray-500"
                  />
                ) : (
