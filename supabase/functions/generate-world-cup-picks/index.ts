...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // The schedule barely changes within a day — cache it per requested date instead of re-running
    // a live grounded search on every single request, including the frontend's 30-minute background
    // refresh from any open tab.
    const isoCodePattern = /^[A-Za-z]{2}$/;
    let matches: ScheduleMatch[];
    const scheduleCacheResponse = await fetch(
      `${supabaseUrl}/rest/v1/world_cup_schedule_cache?date=eq.${date}&select=matches`,
      {
        headers: {
    ...
    if (scheduleCacheRows.length > 0 && Array.isArray(scheduleCacheRows[0].matches)) {
      ...
      await fetch(`${supabaseUrl}/rest/v1/world_cup_schedule_cache?on_conflict=date`, {
        method: "POST",
        headers: {
          apikey: serviceRoleKey,
          Authorization: `Bearer ${serviceRoleKey}`,
