...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let sportsbookGroup: string | undefined;
    ...
    if (req.method !== "GET") {
      try {
        ...
        sportsbookGroup =
          typeof body?.sportsbookGroup === "string" ? body.sportsbookGroup : undefined;
      } catch (_) {
    ...
    const groupValue = url.searchParams.get("sportsbookGroup") ?? sportsbookGroup;
    const includeSuffix = includeValue ? `?include=${includeValue}` : "";
    const basePath = groupValue
      ? `https://api.sportsdata.io/v3/nba/odds/json/BettingPlayerPropsByGameID/${gameId}/${groupValue}`
      : `https://api.sportsdata.io/v3/nba/odds/json/BettingPlayerPropsByGameID/${gameId}`;
    const apiUrl = `${basePath}${includeSuffix}`;
    ...
    return new Response(JSON.stringify({ gameId, sportsbookGroup: groupValue, data }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error) {
