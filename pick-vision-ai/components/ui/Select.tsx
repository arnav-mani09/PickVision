...
export const Select: React.FC<SelectProps> = ({ label, id, options, className = '', ...props }) => {
  ...
  return (
    <div className="w-full">
      ...
      <select
        id={selectId}
        className={`block w-full bg-gray-700 border-gray-600 text-gray-100 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm p-2 h-[42px] ${className}`}
        {...props}
      >
        {options.map(option => (
